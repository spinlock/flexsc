  ...
  /* Special case access to the first 2nd-level block.  This is the
    ...
    if (__builtin_expect (key < PTHREAD_KEY_2NDLEVEL_SIZE, 1)) {
        ...
        if (likely(flexsc_enabled())) {
            int *specific_usedp;
            level2 = flexsc_specific_1stblock(key, &specific_usedp);
            if (value != NULL) {
                *specific_usedp = 1;
            }
        }
        ...
        if (likely(flexsc_enabled())) {
            int *specified_usedp;
            level2 = flexsc_specific_mtxblock(idx1st, idx2nd, &specified_usedp); 
            if (level2 == NULL && value == NULL) {
                return 0;
            }
            return ENOMEM;
            *specified_usedp = 1;
            ...
            if (level2 == NULL) {
                ...
                level2 = (struct pthread_key_data *) calloc (PTHREAD_KEY_2NDLEVEL_SIZE,
                                                             sizeof (*level2));
                if (level2 == NULL)
                    return ENOMEM;
